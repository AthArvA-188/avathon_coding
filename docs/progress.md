...
- [x] Production forecast: 124 series × 52 wks; horizon P50 total **952,860 units vs 896,000 annual capacity** → structurally supply-constrained year (headline for deck)
...
- [x] **Quantile-driven planning (D33, 2026-08-22)** — `--quantile p90/p10` builds the MPS/heuristic demand cube from any forecast quantile through one whitelisted parameter on `mps.load_demand()`, threaded through both solvers AND the validators (which replay at the same quantile the plan was solved on). Plans persist beside the P50 set (`baseline_p90`, `heuristic_p90`, `baseline_p10`); the plan page grew a picker with quantile-aware labels. Measured on the shipped DB (all validators PASS): P90 high case is unmeetable — 33,067 u unmet with buffers at 0.2 median WOS and $5.55M freight (greedy: 152,809 u unmet); P10 low case is comfortable — 0 unmet, $3.58M freight, 7.0 median WOS. Stale prior-session `p10`/`p90` experiment rows dropped and re-solved through the audited path. 86 tests green (4 new: SQL-injection whitelist, cube ordering, default-is-p50, P90 SQL reconciliation; 1 pre-existing test made hermetic vs a real key in the shell).

- [x] **Sanitize refusals surfaced with reasons (D32 follow-up, 2026-08-22)** — found by the owner using the drop zone: a photographed message with a demand shock on V5 was read perfectly by vision and refused by the core-variants-only policy, but the UI just said "0 new pending events" — a correct refusal that read as a bug. `sanitize()` now records a human-readable policy reason per dropped event, threaded through `extract`/`extract_image`/`extract_inbox` (all backward-compatible out-params) into the on-screen extraction note, the `--signals` CLI (`REFUSED: …` lines), and the JSON summary (`rejected: {file: [reasons]}`); files read with no planning content at all say that instead. 88 tests green (2 new: reason text for the V5 policy, per-file rejected_out plumbing).

- [x] **Drag-and-drop image upload on the Signals page (D32, 2026-08-22)** — the D30 image path is now in-app: a drop zone (drag or click, multi-file) sends base64-in-JSON to the same `/api/inbox` POST — deliberately NOT multipart, which is a CSRF "simple request" that would reopen the round-5 hole. Server sniffs magic bytes (png/jpg/webp/gif), requires the extension to match the actual bytes (the engine picks the vision media type from it), caps at 5 MB before decoding, and applies the existing safe-name/fixture guards. Verified live end to end: 7 rejection paths + a generated carrier-advisory PNG → claude-sonnet-5 vision → one pending `freight_disruption` (Geo G2 / Air / 2024W10–W12) at the capped 0.75 confidence awaiting targeted approval.

- [x] **In-app usage guides + UI human gate (D31, 2026-08-22)** — per owner direction, "how to drive the app" lives inside the app: every page opens with a "How to use this page" strip (DO / WATCH FOR) beside the provenance strip, and the home page carries "Before you start", the 60-second demo script and troubleshooting (the separate playthrough artifact is superseded). First (and only) UI write surface: approve/reject a pending signal per-row (same UPDATE as `--approve-signal <id>`), create an inbox message with real extraction (`engine/extract_only.py`), delete an unprotected inbox file (pending rows only; eval fixtures immutable; audit rows survive). Planner what-ifs prefill an editable inbox message. Full CRUD cycle verified live incl. traversal/fixture-protection guards; plan tables remain read-only from the app. Adversarial review round 5 (15-agent find→refute) confirmed 10 defects — CSRF/cross-origin actuation of the human gate, a rejection-dodge via re-adding content under a new filename, Windows reserved device names, an over-counting extraction summary, offline single-variant-uplift parsing, an inverted what-if direction, two stale check-counts, a stale forecast headline (952,865→952,860), an over-eager planner send-guard, and silent client fetch failures — all fixed with regression tests; 82 tests green.
